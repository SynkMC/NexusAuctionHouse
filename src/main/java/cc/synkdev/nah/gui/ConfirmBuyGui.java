...
public class ConfirmBuyGui {
    ...
    public Gui gui(Player p, BINAuction bA) {
        Gui gui = Gui.gui()
                .title(Component.text(ChatColor.YELLOW+ Lang.translate("confirmBuy")))
                .rows(4)
        ...
        return ItemBuilder.from(item).asGuiItem(event -> {
            ...
            if (!bA.getSeller().getUniqueId().toString().equalsIgnoreCase(pl.getUniqueId().toString())) {
                if (core.getEcon().has(pl, bA.getPrice())) {
                    ...
                    if (bA.getSeller().isOnline()) bA.getSeller().getPlayer().sendMessage(core.prefix()+ChatColor.GOLD+ pl.getName()+" "+Lang.translate("smnBought", bA.getPrice()+""));
                    ...
                    pl.sendMessage(core.prefix() + ChatColor.GREEN + Lang.translate("successBuy", bA.getSeller().getName(), bA.getPrice()+""));
                } else pl.sendMessage(core.prefix()+ChatColor.RED+Lang.translate("notEnoughBuy"));
            } else pl.sendMessage(core.prefix()+ChatColor.RED+Lang.translate("buyFromYou"));
            pl.closeInventory();
        });
    }
    GuiItem cancel() {
